...
class App {
  ...
  cache_for(HttpRequest req, HttpResponse res, [content, nextFilter]) {
    ...
    res.headers.add(HttpHeaders.EXPIRES, new Date.fromMillisecondsSinceEpoch(exp, isUtc: true));
    return content;
  }
...
class AppRequest implements HttpRequest {
  ...
  HttpSession session([init(HttpSession session)]) => httpRequest.session();
